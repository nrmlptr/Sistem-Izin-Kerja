<?php 
    ...
    public function view_by_month($month, $year){
        ...
        return $this->db->get('iks')->result(); // Tampilkan data transaksi sesuai bulan dan tahun yang diinput oleh user pada filter
    }
        
    public function view_by_year($year){
            $this->db->where('YEAR(wkt_mulai)', $year); // Tambahkan where tahun
            
        return $this->db->get('iks')->result(); // Tampilkan data transaksi sesuai tahun yang diinput oleh user pada filter
    }
        
    public function view_all(){
        return $this->db->get('iks')->result(); // Tampilkan semua data transaksi
    }
        
        public function option_tahun(){
            $this->db->select('YEAR(wkt_mulai) AS tahun'); // Ambil Tahun dari field tgl
            $this->db->from('iks'); // select ke tabel transaksi
